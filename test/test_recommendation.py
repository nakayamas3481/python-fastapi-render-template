import os
from ai import get_recommendation, ingest_resume, ingest_resume_for_recommendataions
...
def test_recommendation_api(db_session, client):
    ...
    print(job_post.id)
    for firtst_name, last_name, email, filename in test_resumes:
        post_data = {
    ...
    assert response.status_code == 200
    data = response.json()
    assert data['firtst_name'] == 'Koudai'
